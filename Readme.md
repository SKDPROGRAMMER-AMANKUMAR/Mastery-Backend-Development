...
## <strong style="color:Blue"> Learn mongoDB aggregation pipelines  , We make a controller called ```getUserChannelProfile``` for fetching ```subscribers``` , ```whome you subscribed``` etc. by using aggreggation pipelines</strong>

Write the Below code in ```user.controller.js``` file , which present inside ```controller``` directory
```javascript
import { asyncHandler } from "../utils/asyncHandler.js";
import { ApiError } from "../utils/ApiError.js";
import { User } from "../models/user.model.js";
...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Something went wrong while generating refresh ad access token "
    );
  }
};
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const { fullName, email } = req.body;
  console.log(
    "The updated details via (updateAccountDetails) are :- ",
    req.body
  );
...
/*// Async handler to retrieve the user's channel profile information*/
const getUserChannelProfile = asyncHandler(async (req, res) => {
  /* // Function logic to get the user channel profile will go here*/

  /*// Destructures the 'username' parameter from the request URL
      // The 'username' is typically passed as part of the route (e.g., '/user/:username') 
      // and is accessed here to use in the controller logic (e.g., fetching user data).*/
  const { username } = req.params;

  /*// Checks if the 'username' is either not provided or consists only of whitespace
  // The optional chaining (?.) ensures that if 'username' is undefined or null, it won't throw an error
  // If 'username' is invalid (empty or just spaces), an ApiError is thrown with a 400 status and a descriptive error message*/
  if (!username?.trim()) {
    throw new ApiError(400, "Username is required");
  }

  /*// Initiates an aggregation query on the 'User' model to retrieve and process data.
  // The 'await' ensures that the asynchronous operation completes before continuing with the execution.
  // 'User.aggregate()' is used to perform an aggregation pipeline, which allows advanced data processing 
  // (e.g., filtering, grouping, sorting) on the 'User' collection in the database.
  // The aggregation pipeline stages (e.g., $match, $group) are typically passed as an array, 
  // and they will be defined within the array that follows this code.*/
  const channel = await User.aggregate([
    // Aggregation stages go here

    /*// The $match stage filters the documents in the aggregation pipeline to include only those 
    // where the 'username' field matches the provided 'username' parameter (converted to lowercase).
    // The optional chaining (?.) ensures that if 'username' is undefined or null, no error will be thrown.
    // This helps in performing a case-insensitive search for the 'username' in the database.*/
    {
      $match: {
        username: username?.toLowerCase(),
      },
    },
    /* // The $lookup stage performs a join between two collections in MongoDB.
        // It allows you to combine documents from the 'User' collection (current collection) 
        // with documents from the 'subscriptions' collection based on specific fields.*/
    {
      $lookup: {
        // 'from' specifies the target collection to join with (in this case, 'subscriptions').
        from: "subscriptions",
        /*// 'localField' is the field from the current collection (here, 'User') 
                // that will be matched against the 'foreignField' in the 'subscriptions' collection.
                // In this case, it uses the '_id' field from the 'User' collection.*/
        localField: "_id",
        /* // 'foreignField' specifies the field in the 'subscriptions' collection 
                // that should match the 'localField' (here, the 'channel' field in 'subscriptions').*/
        foreignField: "channel",
        // 'as' defines the name of the new array field to add to the documents returned.
        // This field will hold the matching documents from the 'subscriptions' collection.
        // The resulting array will be stored in the 'subscribers' field of each document.
        as: "subscribers",
      },
    },
    {
    ...
    {
      // The $addFields stage adds new fields or modifies existing fields in the documents returned by the aggregation pipeline.
      $addFields: {
        /*// 'subscriberCount' calculates the total number of subscribers by getting the size of the 'subscribers' array.
        // The $size operator returns the number of elements in the array, which represents the number of subscribers. */
        subscriberCount: {
          $size: "$subscribers",
        },
         // 'channelsSubscribedToCount' calculates the number of channels the user is subscribed to by getting the size of the 'subscribedTo' array.
        // The $size operator counts the elements of the 'subscribedTo' array, which represents the channels the user follows.
        channelsSubscribedToCount: {
          $size: "$subscribedTo",
        },
         // 'isSubscribed' is a boolean field that checks if the user is subscribed to the channel.
        // The $cond operator is used for conditional logic. It evaluates whether the user’s _id is present in the 'subscribers.subscriber' array.
        // If the user’s _id is found in 'subscribers.subscriber', it sets 'isSubscribed' to true, otherwise false.
        isSubscribed: {
          $cond: {
            // $in checks if the user's _id is present in the 'subscribers.subscriber' array.
            if: { $in: [req.user?._id, "$subscribers.subscriber"] },// Check if user._id exists in 'subscribers.subscriber'
    ...
    {
        // The $project stage is used to include, exclude, or reshape fields in the documents that pass through the aggregation pipeline.
    // It allows you to specify which fields should be included or excluded in the final output.
        $project:{
          fullName:1,  // Includes the 'fullName' field in the output documents (1 means included).
          username:1, // Includes the 'username' field in the output documents.
          email:1,// Includes the 'email' field in the output documents.
          avatar:1,// Includes the 'avatar' field (typically the user's profile image) in the output documents.
          coverImage:1,// Includes the 'coverImage' field (the user's cover image) in the output documents.
          subscriberCount:1,// Includes the 'subscriberCount' field (the number of subscribers) in the output documents.
          channelsSubscribedToCount:1, // Includes the 'channelsSubscribedToCount' field (the number of channels the user is subscribed to) in the output documents.
          isSubscribed:1,// Includes the 'isSubscribed' field (whether the user is subscribed to the channel or not) in the output documents.

        }
    }
  ]);

  console.log("Data achived by (channel) aggregation pipeline is:- ",channel);
...
// The response includes a JSON object created using the ApiResponse class, which contains:
// - A status code (200) indicating a successful request.
// - The 'channel[0]' data, which is the first item from the 'channel' array (likely the user's channel profile).
// - A message indicating the successful retrieval of the user's channel profile.*/
  return res
  .status(200)// Sends a successful HTTP response with status code 200.
  .json(new ApiResponse(200, channel[0], "User's channel profile retrieved successfully"));
...
## <strong style="color:Gold">We are made a controller called ```getWatchHistory``` by using ```Aggregation Pipeline``` </strong>
Write the code below in ```user.controller.js``` file which present in ```controller``` directory
```javascript
import { asyncHandler } from "../utils/asyncHandler.js";
import { ApiError } from "../utils/ApiError.js";
import { User } from "../models/user.model.js";
...
/*// Asynchronous function to retrieve the watch history of a user.
// Wrapped with 'asyncHandler' to handle errors in asynchronous operations automatically.
// 'req' contains the request data, including user authentication or query parameters.
// 'res' will be used to send the response back to the client.*/
const getWatchHistory = asyncHandler(async(req,res)=>{
   ...
   /*// Performs an aggregation operation on the 'User' collection to retrieve and process user data.
// The 'await' keyword ensures the aggregation completes before the next line executes.
// The aggregation pipeline, passed as an array, defines the stages for filtering, transforming, and shaping the data.*/
   const user = await User.aggregate([

    /*// The $match stage filters the documents in the aggregation pipeline to include only those
    // that match the specified condition. It ensures that only the document corresponding to the
    // authenticated user's ID is included in further stages of the pipeline.*/
    {
      $match: {
        /*// Matches documents where the '_id' field equals the authenticated user's ID.
        // 'req.user._id' is the ID of the currently authenticated user, extracted from the request object.
        // 'new mongoose.Types.ObjectId()' converts the user's ID into an ObjectId format required by MongoDB for comparison.*/
        _id: new mongoose.Types.ObjectId(req.user._id),
      }
    },
    // Performs a multi-stage $lookup operation to enrich the user's watch history with detailed video and owner information.
    {
      $lookup: {
         // Specifies the 'videos' collection to join with the current documents in the pipeline.
        from: "videos",
         // Matches the 'watchHistory' field in the current document to the '_id' field in the 'videos' collection.
        localField: "watchHistory", // Array of video IDs in the user's watch history.
        foreignField: "_id", // Field in the 'videos' collection that corresponds to the video IDs.

        /*// Renames the resulting array from the lookup to 'watchHistory', replacing the original field.
    as: "watchHistory",*/
        as: "watchHistory",
        // Adds an inner pipeline to further process the matched 'videos' data.
        pipeline: [
          // First-level $lookup: Fetches the owner details for each video in the user's watch history.
          {
            $lookup: {
              from: "users", // Specifies the 'users' collection for the second-level join.
              localField: "owner",// Matches the 'owner' field in the 'videos' collection.
              foreignField: "_id",// Matches the 'owner' field in the 'videos' collection.
              as: "owner", // Stores the resulting data in the 'owner' field.

              /* // Adds a pipeline to limit the fields fetched for the video owner details.*/
              pipeline: [
                {
                  $project: {
                    fullName: 1,// Includes the 'fullName' of the owner in the result.
                    username: 1, // Includes the 'username' of the owner in the result.
                    avatar: 1 // Includes the 'avatar' of the owner in the result.
                  }
                }
              ]
            }
          },
          /*// Adds a new field or modifies an existing field in the documents using the $addFields stage.*/
          {
            $addFields:{
              // The 'owner' field is updated to store the first element of the 'owner' array.
              // This is useful when the 'owner' field is an array (e.g., after a $lookup operation) and only the first entry is required.
              owner: {
                $first: "$owner"// Extracts the first element from the 'owner' array and assigns it to the 'owner' field.
              }
            }
          }
        ]
   ...
   .json(
    new  ApiResponse(
      200,
      user[0].watchHistory, // Retrieves the 'watchHistory' array from the first user document.
...
### <strong style="color:red">Here we add all the ```routes``` of ```user.controller.js``` file in ```user.routes.js```</strong>
```javascript
  import { Router } from "express";
import {
  loginUser,
...
router.route("/c/:username").get(verifyJWT, getUserChannelProfile); //We write it different because , We accept data from "url"
...
# <strong style="color:red;">This is the updated code of ```user.controller.js``` file , Where all the routes that Hitesh Sir explain work perfectly , </strong>
```javascript
import { asyncHandler } from "../utils/asyncHandler.js";
import { ApiError } from "../utils/ApiError.js";
import { User } from "../models/user.model.js";
